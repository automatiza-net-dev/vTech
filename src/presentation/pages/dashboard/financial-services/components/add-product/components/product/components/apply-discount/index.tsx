  ...
  return (
    <S.ApplyDiscount>
      ...
      <div className="row">
        ...
        <InputCurrency prefix=" " name="discount" label="Desconto" onChangeMode="blur" />
  ...
  onClick={() => {
    ...
    if (discountType === "percent") {
      const updatedCart = cart.map(item => {
        ...
        const totalDiscount = (quantity * unitPrice * discountValue) / 100;
    ...
    if (discountType === "fixed") {
      ...
      const updatedCart = cart.map(item => {
        ...
        const quantity = Number(variation.quantity) || 0;
        const unitPrice = variation.unitaryValue || 0;
        const totalDiscount = (quantity * unitPrice * equivalentPercent) / 100;

        return {
          ...item,
          variations: [{
            ...variation,
            total:  Math.round((variation.unitaryValue  - totalDiscount) * 100) / 100,
