...
import UpsertDepartment from "./Create";
		...
		<PageWrapper title="Controle de departamentos">
			<Container>
				...
				<Table
					...
					dataSource={departmentsQuery.data?.map((d) => ({
				...
				<Modal
					...
					title={
						typeof createFormState === "string"
							? "Cadastrar departamento"
							: "Editar departamento"
					}
					open={createFormState !== "closed"}
					onClose={() => setCreateFormState("closed")}
				>
					<UpsertDepartment
						initialData={
							typeof createFormState === "string" ? undefined : createFormState
						}
						shouldRefresh={() => {
							departmentsQuery.refetch();
						}}
