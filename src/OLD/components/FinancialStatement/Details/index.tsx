...
function BorderoDetails({
	...
	const processBorderos = useCallback(() => {
		if (finances.length === 0 || titles.length === 0) {
			return;
		}

		financesService
			?.createBorderoItems({
				type: finances[0].type === "CREDITO" ? "Credito" : "Debito",
	...
	return (
		<Container className="uk-padding-small">
			<h3 className="uk-margin-remove">Agrupamento de títulos</h3>
			...
			>
				...
				<div>
					...
					<Input disabled value={finances[0]?.tef_adquirente} />
				</div>
				<div>
